...
import { accessRequests as accessRequestsApi, deals as dealsApi } from '@/lib/api';
...
const adminNav = [
  ...
  { name: 'Deals',              href: '/admin/deals',           icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z', color: 'from-emerald-500 to-green-600', dealBadge: true },
...
export default function AdminSidebar() {
  ...
  useEffect(() => {
    accessRequestsApi.pendingCount()
      ...
      .catch(() => {});
    dealsApi.list()
      .then(all => setPendingDealsCount(all.filter((d: any) => d.status === 'PENDING_VERIFICATION').length))
      .catch(() => {});
  }, []);
  ...
  const NavItem = ({ item, onClick }: { item: typeof adminNav[0]; onClick?: () => void }) => {
    ...
    const badgeCount = (item as any).badge ? pendingAccessCount : (item as any).dealBadge ? pendingDealsCount : 0;
    return (
